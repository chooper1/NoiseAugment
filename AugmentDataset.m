function AugmentDataset(paths, positions, noise_settings, other_noise, goal_freq)
    ...
    for i=1:length(noise_paths) % could be an issue for this to all be loaded in memory at once
        noise_Files{i}(:) = [dir(fullfile(noise_paths(i),'*.wav')), dir(fullfile(noise_paths(i),'*.flac'))]; %gets all wav files in directory
    ...
    for j=1:length(dataset_path)
    	dataset_Files = dir(fullfile(dataset_path(j),'*.wav')); %gets all wav files in directory
        for k = 1:length(dataset_Files)
            ...
            augment_file(dataset_filename, noise_filename, H1, H2, noise_index, dataset_path(j), output_path, name_conv, k, amp, other_noise, goal_freq,source_pos_temp, noise_pos_temp);
            k
